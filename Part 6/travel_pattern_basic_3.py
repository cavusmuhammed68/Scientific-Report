...
for i, (column, title) in enumerate(zip(columns_of_interest.keys(), figure_titles)):
    ...
    for region in combined_data['region'].unique():
        ...
        counts = region_data[column].value_counts(normalize=True).sort_index()
        for idx, value in counts.items():
            percentage = f"{value * 100:.1f}%"
            axes[i].text(
                x=list(combined_data['region'].unique()).index(region),
                y=idx,
                s=percentage,
                ha='center', va='center', fontsize=14, fontweight='bold', color='black', zorder=3
            )
...
# Save the figure
save_path_additional = r'C:\Users\cavus\Desktop\Dilum_Ekip_Paper\New Study\Part 6\travel_pattern_with_percentage_bold.png'
